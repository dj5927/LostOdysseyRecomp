...
cmake -S "$WORK_SRC" -B "$BUILD_DIR/NATIVE" -G Ninja     -DCMAKE_BUILD_TYPE=Release     -DCMAKE_C_COMPILER=/usr/bin/gcc     -DCMAKE_CXX_COMPILER=/usr/bin/g++     -DCMAKE_AR=/usr/bin/ar     -DCMAKE_RANLIB=/usr/bin/ranlib     -DCLANG_FORMAT_EXE="$CLANG_FORMAT"     -DLLVM_TARGETS_TO_BUILD=None     -DLLVM_ENABLE_EH=ON     -DLLVM_ENABLE_RTTI=ON     -DLLVM_INCLUDE_TESTS=OFF     -DLLVM_INCLUDE_DOCS=OFF     -DLLVM_INCLUDE_EXAMPLES=OFF     -DHLSL_INCLUDE_TESTS=OFF     -DHLSL_OPTIONAL_PROJS_IN_DEFAULT=OFF     -DHLSL_COPY_GENERATED_SOURCES=1

cmake -S "$WORK_SRC" -B "$BUILD_DIR" -G Ninja     -DCMAKE_BUILD_TYPE=Release     -DCMAKE_TOOLCHAIN_FILE="$TOOLCHAIN"     -DCLANG_FORMAT_EXE="$CLANG_FORMAT"     -DENABLE_SPIRV_CODEGEN=ON     -DLLVM_TARGETS_TO_BUILD=     -DLLVM_DEFAULT_TARGET_TRIPLE=dxil-ms-dx     -DLLVM_ENABLE_EH=ON     -DLLVM_ENABLE_RTTI=ON     -DLLVM_INCLUDE_TESTS=OFF     -DLLVM_INCLUDE_DOCS=OFF     -DLLVM_INCLUDE_EXAMPLES=OFF     -DHLSL_INCLUDE_TESTS=OFF     -DHLSL_OPTIONAL_PROJS_IN_DEFAULT=OFF     -DSPIRV_BUILD_TESTS=OFF     -DSPIRV_SKIP_EXECUTABLES=ON     -DHLSL_COPY_GENERATED_SOURCES=1
