...
The primary inventory's `shaders` object maps names to [FPD path, container
offset]. Optional CPX inventory packages identify decoded original containers
by SHA-256 and size. No runtime shader cache is accepted or consulted.
The output contains layouts, fetch metadata, sizes and
...
# Original SDK semantic metadata plus the observed packed static-mesh layout
# (f16385, VS 1474db97dfc0afad). These are declarations, never captured code.
# Packed tangent/binormal/normal flags are capture-derived; unlike DECLARATIONS
# this does not claim recovery of a constant title constructor.
PACKED_STATIC_ELEMENTS = (
    (0x002a23b9, 0, 0, 0, 0),
    (0x001a2286, 16, 0, 6, 0),
    (0x001a2286, 20, 0, 7, 0),
    (0x001a2286, 24, 0, 3, 0),
...
def load_inventory(inventory_path, cpx_inventory=None):
    inventory = dict(json.loads(inventory_path.read_text(encoding='utf-8'))['shaders'])
    if cpx_inventory is None:
        return inventory
    decoded = json.loads(cpx_inventory.read_text(encoding='utf-8'))
    require(not decoded.get('failures'), 'CPX inventory contains failed decodes')
    for package in decoded['packages']:
        path = Path(package['decoded_file'])
        data = path.read_bytes()
        require(len(data) == package['decoded_size'] and
                hashlib.sha256(data).hexdigest() == package['decoded_sha256'],
                f'Decoded CPX identity mismatch: {path}')
        for shader in package['shaders']:
            name, offset = shader['name'], shader['container_offset']
            require(re.fullmatch(r'(vs|ps)_[0-9a-f]{16}\.bin', name) and
                    isinstance(offset, int) and 0 <= offset <= len(data) - 36,
                    'Invalid CPX shader location')
            # Validate before merging: an existing raw source must not hide a
            # corrupt CPX record. Only complete original SDK containers qualify.
            if name.startswith('vs_'):
                current = read_source(path, offset, name)
                if name in inventory:
                    require(current == read_source(*inventory[name], name),
                            f'Conflicting original VS metadata: {name}')
            inventory.setdefault(name, [str(path), offset])
    return inventory
...
def generate(image_path, inventory_path, cpx_inventory=None):
    ...
    declarations.append((None, len(elements), len(PACKED_STATIC_ELEMENTS), 10))
    elements.extend(PACKED_STATIC_ELEMENTS)
    semantics.append({(usage, index) for _, _, _, usage, index in PACKED_STATIC_ELEMENTS})
    rows, fetches = [], []
    inventory = load_inventory(inventory_path, cpx_inventory)
    ...
    require(rows and len(rows) <= 65535, 'Source metadata exceeds table index range')
    ...
    lines.extend(f'    {{{first}, {count}, {extent}}}, // ' +
                 (f'Original constructor 0x{entry:08X}' if entry is not None else
                  'Observed packed static mesh; original SDK semantic mapping')
                 for entry, first, count, extent in declarations)
...
def generate_links(inventory_path, fixed_metadata, cpx_inventory=None):
    ...
    require(names and len(names) == len(set(names)) and len(names) <= 65535,
            'Invalid fixed source metadata set')
    sources, pixels = {}, {}
    inventory = load_inventory(inventory_path, cpx_inventory)
    # Preserve existing raw-package pixel representatives. A newly discovered
    # CPX equivalent must not make historical link plans depend on a new file.
    primary_names = json.loads(inventory_path.read_text(encoding='utf-8'))['shaders']
    for name in sorted(inventory, key=lambda name: (name not in primary_names, name)):
        path, offset = inventory[name]
...
def main():
    ...
    parser.add_argument('--cpx-inventory', type=Path,
                        help='Decoded original CPX package inventory with SHA-256, sizes and SDK offsets')
    args = parser.parse_args()
    metadata = generate(args.image, args.inventory, args.cpx_inventory)
    ...
    if args.linked_header:
        linked = generate_links(args.inventory, metadata, args.cpx_inventory)
        digest = hashlib.sha256((metadata + linked).encode('utf-8')).hexdigest()
        identity_path = args.linked_header.with_name('resource_variant_identity.h')
        identity = ('// Generated discovery identity; individual shader cache keys are unchanged.\n'
                    '#pragma once\n#include <string_view>\n'
                    'namespace xenos::resources::variants {\n'
                    f'inline constexpr std::string_view DiscoveryIdentity = "{digest}";\n'
                    '}\n')
        if args.check:
            require(args.linked_header.read_text(encoding='utf-8') == linked, 'Generated link metadata differs')
            require(identity_path.read_text(encoding='utf-8') == identity, 'Generated discovery identity differs')
        else:
            args.linked_header.write_text(linked, encoding='utf-8', newline='\n')
            identity_path.write_text(identity, encoding='utf-8', newline='\n')
    print('Verified original source hashes and declaration metadata; no microcode emitted')
