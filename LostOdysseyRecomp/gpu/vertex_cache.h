...
namespace gpu::geometry_prepare
{
    ...
    struct IndexEntry
    {
        ...
        // Sampled source guest bytes the conversion was built from.
        SampledContent content;
    ...
    template<class Entry, class Key = uint64_t, class Hash = ankerl::unordered_dense::hash<Key>>
    class FrameCache
    {
        using Map = ankerl::unordered_dense::map<Key, Entry, Hash>;
        Map entries;
        size_t capacity;
    ...
    public:
        ...
        explicit FrameCache(size_t limit) : capacity(std::max(size_t(1), limit))
        {
            entries.reserve(capacity);
        }
        auto begin() { return entries.begin(); }
        auto end() { return entries.end(); }
        auto find(const Key& key) { return entries.find(key); }
        auto erase(typename Map::iterator it) { return entries.erase(it); }
        size_t size() const { return entries.size(); }
        size_t bucket_count() const { return entries.bucket_count(); }
        uint64_t Evictions() const { return evictions; }

        void emplace(const Key& key, Entry&& entry)
        {
    ...
    class VertexCache : public FrameCache<VertexEntry>
    {
    public:
        static constexpr size_t kCapacity = 65536;
        explicit VertexCache(size_t limit = kCapacity) : FrameCache<VertexEntry>(limit) {}
    };
    ...
    class IndexCache : public FrameCache<IndexEntry, IndexKey, IndexKeyHash>
    {
    public:
        static constexpr size_t kCapacity = 4096;
        static constexpr uint32_t kMinCount = 256;
        explicit IndexCache(size_t limit = kCapacity) : FrameCache<IndexEntry, IndexKey, IndexKeyHash>(limit) {}
    };
}
