...
struct LiveOptions {
    ...
    int stationary=1,coverage=1,acceptance=0,mv_debug=0,mv_consume=1,snap_stationary=0,stationary_color_clip=0;
...
#define LO_LIVE_FIELD(name) else if(key==#name)ok=read(value.name)
        ...
        LO_LIVE_FIELD(stationary_color_clip);
...
#undef LO_LIVE_FIELD
    ...
    for(int flag:{value.hdr,value.materials,value.stationary,value.coverage,value.mv_debug,value.mv_consume,value.snap_stationary,value.stationary_color_clip})
        if(flag<0||flag>1)return bad("Invalid boolean control");
    if(value.acceptance<0||value.acceptance>2||!range(value.history_weight,0,.95f)||!range(value.stationary_weight,0,.95f)
