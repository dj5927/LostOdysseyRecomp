...
The user's same-scene result was partial: the upper large robot is now stable, while the lower enemy eyes still flicker. The candidate therefore remains unpublished and the lower-eye defect remains open. The follow-up diagnostic retains the AA3 condition while removing the transient `temporalJitter` gate and adds process-scoped AA, jitter, history and bloom controls plus optional geometry/resolve tracing. An eight-frame trace showed history reuse without gaps, and the frame-7208 capture found four compared draw pairs with identical VP, world, active-bone, vertex-buffer and index data, excluding a CPU upload mismatch for those pairs. Cross-backend comparison covered four 84-index pairs; the later report includes additional 144-index eyes outside that capture, so complete scene coverage is not established. The latest live diagnostic control run is pending the user's same-scene validation. Detailed evidence is in [the TAA bloom prefilter note](notes/taa-bloom-prefilter.md); prior fixture results are reused.
