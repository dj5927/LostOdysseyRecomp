...
D3D12 仍是可用基线。Windows Vulkan 已有 RTX 5080 实机场景的界定验证，与更广 GPU 和全游戏覆盖分开。Direct3D 11、Linux/Steam Deck 和 Switch 是彼此独立的平台工作，未声明完成或发布日期。Linux 首可玩范围已实现：在宿主 Mesa + SDL2 + X11/XWayland 上，以 `./LostOdysseyRecomp --game <disc>` 启动 Vulkan-only 未打包 ELF。WSL2 Manjaro + Mesa Dozen 运行已进入 1280x720 Vulkan 窗口，用户观看后关闭并正常退出；原生 Linux ICD、Steam Deck 和更广游戏流程覆盖仍开放。Linux 安装器／导入器、XDG/Flatpak 用户路径映射、POSIX libcurl 更新器、AppImage 打包（`tools/package_appimage.py`）以及 Flatpak 清单已在工作区实现，并已添加 CI 发布生成；原生 Steam Deck sniper 运行时打包、Flathub 提交以及 Win32 专属的首开／F1 调试菜单仍待后续跟进。现已支持快速按需 Linux 构建（`tools/build_linux.sh` / `tools/build_wsl.bat`）与跨平台便携式 Vulkan 着色器包（`.lospv`，在 WSL2 Linux 下实测 28,482 个着色器 1.2 秒零编译启动）。见 [Linux 移植评估](notes/linux-port-evaluation-2026-09-13.md)。
...
- [~] **Linux 首可玩与便携式着色器包：**Vulkan-only 未打包 ELF 已在 WSL2 Manjaro 上通过 `linux-clang` 编译，配合 `shaders/portable_vk.lospv`（28,482 个着色器压缩为 169.9 MB）实现 1.2 秒零编译瞬时启动与 0 次 DXC 调用，消除了运行期编译卡顿。通过 `tools/build_wsl.bat` 接入了快速按需构建流程。这仍是有界 WSL 证据，不是原生 Linux ICD、Steam Deck、正式打包、全游戏或已发布 Linux 支持。公开 Windows v0.5.14 ZIP 不包含 Linux。
