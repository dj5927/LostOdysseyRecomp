...
- [~] **更新器简化：**v0.5.7 发布准备使用 main 候选 6112c06。PPC 同步已复用已验证的四分片库，CI key 为 3260d975，未运行 PPC 构建。CI、main 推送、tag、安装包与公开 Release 均待完成。既有 synthetic 证据包括 standalone 44/44、LoUpdaterTest 和 HelperContext；尚无真实更新交易、公开下载、可见提示交互或玩家验收。
...
- [~] **坎托族魔装兵光照闪烁：**用户已验收第五版在报告场景中不闪了；核对 PID 26188、候选 EXE、源码 0.5.6-hotfix1、Test-TAA-Lighting-5.cmd、HDR 默认 0 和 materials 默认 1。本验收仅覆盖该报告遭遇。v0.5.7 发布准备使用 main 候选 6112c06；PPC 同步已就绪，CI、main 推送、tag、安装包与公开 Release 均待完成。
...
- [~] **Vulkan 像素着色器 LoopEnd 谓词退出：**v0.5.7 发布准备使用 main 候选 6112c06，复用既有 1d139c9 对齐和 fixture 证据，未重跑检查。有界结构修复优化 9 个既有 predLoop shader 中的 8 个；guard、64-lane GPU/reference 与 cache 检查通过。复杂循环、游戏验证、玩家验收和实际 GPU 功耗／跨场景证据仍待完成。PPC 同步已就绪；CI、main 推送、tag、安装包与公开 Release 均待完成。
- [~] **相邻重复 resolve copy 消除：**v0.5.7 发布准备使用 main 候选 6112c06，复用既有对齐实现和 fixture 证据，未重跑检查。只消除源、目标 allocation、format 与 region 相同且立即相邻的 copy，并在干扰时失效。CPU 与 D3D12/Vulkan FP16 fixture 已通过；实际游戏命中、跨场景验证和玩家验收仍待完成。PPC 同步已就绪；CI、main 推送、tag、安装包与公开 Release 均待完成。
