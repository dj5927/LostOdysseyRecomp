...
- [~] **超宽屏 (21:9) 与 FOV 布局（Issue #17）：**实验性原生 Hor+ 超宽屏渲染已在 commit `fa38df7` 中初步实现，当前仅支持 3440×1440 分辨率，覆盖 frame-plan、catalog、过场影片、HUD 布局与分辨率选项。Windows Clang 构建通过；`FramePlan`（18 项检查）、`TargetMapping`（5 项检查）、分辨率计算（40 项检查）及时序抖动/相位测试均通过。Vulkan 内置 28,482 个着色器启动并在 13.93 秒加载存档；场景转换截图 382/383 以及主颜色/深度 3440×1472（含 padding）resolve 至 3440×1440 均获验证。超宽屏下阴影映射异常属于已知且用户报告的问题。完整稳定实机游玩、Hor+ 画面视觉、HUD、阴影映射修复、动态切分辨率、故障注入及其它后端验证仍待用户手动测试与视觉验收；正在准备 v0.6.6 版本发布（尚未发布），Issue #17 保持打开且处于进行中状态。
