...
- [~] **更新器简化：**用户声明的源码和发布目标为 `v0.5.6-hotfix1`，目前尚未发布。本地实现已移除“有更新时”对已安装 release manifest 和 EXE hash 的阻断，保留安全解压、回滚及退出后替换。standalone 与游戏启动路径会把空安装识别为版本 `0.0.0`；成功安装后 helper 先询问是否启动，默认 No，silent、失败及启动失败路径均不启动游戏。复用 source-0.5.6 的隐藏 synthetic 检查：standalone 44/44；`LoUpdaterTest` 的 version/asset/integrity/staging/rollback/helper/preservation；以及 Unicode cwd 下 `HelperContext` 的空安装且不擅自启动。为在获取旧包时保留同意机制，使用本地复制的 helper。行为证据未变，故未重新构建或重跑测试。尚未记录真实游戏、公开下载、可见提示交互、玩家验收或发布；验证仍待完成。
...
- [~] **坎托族魔装兵红眼闪烁：**有界的 AA3 Bloom 候选已通过 D3D12/Vulkan fixture 和 captured-input GPU 检查。第三个候选只移除瞬时 `temporalJitter` gate，保留 AA3；用户已确认上方大机器人稳定，但下方敌人的眼睛仍会闪烁。4 个 `f7208`、84-index 眼睛 pair 的 CPU upload／D3D replay bit-identical，Vulkan 在 strict EQUAL mask 下的全部 32 个 jitter phase 也一致，因此未声明 invariant、threshold 或 shader map 修复。供用户在同一场景运行的 process-scoped live diagnostic build 已就绪；其 runtime-control 验证、下方眼睛修复和玩家验收仍待完成。尚未包含在发布版本中。120 FPS 是配置目标，并非测得性能结果。
...
- [~] **Vulkan 像素着色器 LoopEnd 谓词退出：**实现已提交，并随 [`1d139c9`](https://github.com/freefrank/LostOdysseyRecomp/commit/1d139c9) 对齐 main、推送至两个远程；复用此前 Release build 与验证记录。有界结构修复优化 9 个既有 predLoop shader 中的 8 个，并保守保留 1 个；guard、64-lane GPU/reference 与 cache 检查通过。生产 SPIR-V 与既有 probe 一致，复用单帧计时/PNG 证据。复杂循环、游戏验证、玩家验收和发布仍待完成。
- [~] **相邻重复 resolve copy 消除：**实现已随同一合并提交推送；只消除源、目标 allocation、format 与 region 相同且立即相邻的 copy，并在所有干扰时失效。CPU 与 D3D12/Vulkan FP16 fixture 已通过；实际游戏命中和跨场景验证仍待完成。
