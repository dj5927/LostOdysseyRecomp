...
- [~] **GitHub Issue 自动分流：**`main` 现已包含 Issue-opened workflow 和本地工具，使用 OpenAI-compatible API 分析新报告并发布有界回复；修复仍需另行确认。仓库 secret 已配置，BOM 处理已修正。API `/models` 检查返回 HTTP 200，chat-completions 返回 16 个总 token（11 input、5 output），11 个 mock 测试和一个 encoding regression 通过，hosted run [34886197607](https://github.com/freefrank/LostOdysseyRecomp/actions/runs/34886197607) 成功。其 Issue #27 dry-run artifact 已审阅且未 POST。真实新 Issue 的自动公开回复行为、分流质量、修复、验证、玩家验收和发布仍待完成。
