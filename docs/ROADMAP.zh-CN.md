...
- [~] **Issue #6 与 #22 启动诊断：**两个 Issue 均保持 OPEN。v0.5.10 发布后的启动日志实现及既有 fixture／日志记录证据保持有效。两个条目现以已授权的 v0.5.11 为发布准备目标；commit、托管 CI 和公开发布尚未完成。实际本地核对显示 D3D12 与 Vulkan 都记录新增字段，D3D12 到 guest swap #181，Vulkan 在确认日志后停止；它只证明日志记录，没有游戏／实景或报告者验收。根因仍未知；#6 以前的 v0.5.6 关联仅为历史。证据：`out/diagnostic-logging-20260914/local-log-check.json` 与 `game-startup-*/runtime.log`。
