...
- 仅限有界验证：真实网络更新下载、现场替换运行中的游戏二进制、UI 中完整执行真实导入以及全游戏游玩仍未验证。游戏画面还原度、真实手柄硬件验收以及整套 UI 视觉验收尚未确立。未打包 AppImage，macOS 未测试，更新器仍仅限 Windows。导入器 POSIX 文件锁已修复，但不代表全平台游戏可用。
